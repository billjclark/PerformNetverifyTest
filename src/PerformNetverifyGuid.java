 ...
 *
 */
public class PerformNetverifyGuid {
	...
	private static ArrayList<String> getAllIdImagesFromDirectory(File directory) {
        ...
        FilenameFilter idFilter = new FilenameFilter() {
        	public boolean accept(File dir, String name) {
        		if (name.toLowerCase().contains(frontSuffix) && name.endsWith(PDF_SUFFIX) == false) {
        			return true;
        		} else {
        			return false;
        		}
	...
	private static ArrayList<String> getVerificationImages(File directory, String verGuid) {
		ArrayList<String> resultList = new ArrayList<String>(1);
		String ver = verGuid.substring(0, verGuid.indexOf(frontSuffix));
		FilenameFilter verificationFilter = new FilenameFilter() {
			public boolean accept(File dir, String name) {
        		if (name.toLowerCase().contains(ver) && !name.toLowerCase().contains(frontSuffix) && name.endsWith(PDF_SUFFIX) == false) {
        			return true;
        		} else {
        			return false;
        		}
		...
		File[] f = directory.listFiles(verificationFilter);
        if(f != null) {
        	...
        	System.out.println(directory.getName() + ver + " : " + NOT_EXISTING);
        	return null;
        }
